...
| ~~**Opacity's per-dab scale** (bug 21)~~ | **Closed 2026-09-24.** Measured on the real Studio Pen (rounds 3–5 had used a lookalike): Blending is `opacity^2.6` per dab, confirmed by a prediction made before the third point was drawn; Glaze is linear and already matched. The engine reproduces the device's density ratios within 1%. What remains is per-brush: our ink pen's spacing lays about a third of the Studio Pen's dabs, so it draws lighter at the same Opacity — a preset decision, not a slider one |
