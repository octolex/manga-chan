...
| ✅ | Rendering style as a brush field: **Blending** (default) and **Glaze** — 2026-09-09. Blending's per-dab scale measured on the Studio Pen and fixed as `opacity^2.6` on 2026-09-24 (bug 21) |
...
| ~~**Opacity's per-dab scale** (bug 21)~~ | **Closed 2026-09-24.** Measured on the real Studio Pen (rounds 3–5 had used a lookalike): Blending is `opacity^2.6` per dab, confirmed by a prediction made before the third point was drawn; Glaze is linear and already matched. The engine reproduces the device's density ratios within 1%. The per-brush remainder was settled on 2026-09-24 by octolex's decision: the default pen's Spacing is now None, as the Studio Pen's is (0.37 → 0.55 ink at 25%, size 14) |
